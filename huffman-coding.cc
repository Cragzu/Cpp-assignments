...
void occurrences(node* &nd, string s) { // find occurrences of characters in a string, put char value pairs into linked list
	...
	{
		cha = s.at(0); // character at point 0, first one in string
		
		while (s.find(cha) != string::npos) // remove all instances of this char from the string
		{	
			num += 1;
			s.erase(s.begin() + s.find(cha)); // delete this instance
		}
...
void tfree(node* nd) { // delete nodes recursively to avoid memory leaks
	
	if (nd != 0) // check if tree is empty
	{
		tfree(nd->left); // recurse to the left
		tfree(nd->right); // recurse to the right
...
int main() {
	...
	string st; // initial string, get input
	getline(cin, st);
	
	node* nd = 0;
	
	occurrences(nd, st); // create linked list from string
	...
	tfree(nd); // delete to remove memory leaks
